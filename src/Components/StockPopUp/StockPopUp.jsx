                  ...
                  {Array.isArray(stock) &&
                    stock.map((stock) => (
                      <tr key={stock._id} className={`${stock.purchaseQuantity <= stock.reOrderQuantity ? 'bg-yellow-100' : ''}`}>
                        ...
                        <td className="text-center">{parseFloat(stock.purchaseQuantity).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td>{stock.purchaseUnit}</td>
                        <td>{stock.category}</td>
                        <td>{stock.brand}</td>
                        <td className="text-center">{parseFloat(stock.purchasePrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td>{stock.storage}</td>
                      </tr>
                    ))}
                </tbody>
